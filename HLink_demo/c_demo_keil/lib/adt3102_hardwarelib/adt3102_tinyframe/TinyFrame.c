...
volatile uint8 g_TfRxBuff[TF_RX_BUF_SIZE];
volatile uint32 g_TfWriteCount;
volatile uint32 g_TfReadCount;
...
TinyFrameRx tinyFramefRx(void)
{
    ...
    while(g_TfReadCount != g_TfWriteCount)
    {
        ret = TF_AcceptChar(&ft_rx, g_TfRxBuff[g_TfReadCount]);

        g_TfReadCount++;
        g_TfReadCount %= TF_RX_BUF_SIZE;
        ...
        if(0 == ret)
        {
            switch(ft_rx.type)
            {
                ...
                case 0x0107:
                case 0x0108:
                    tempdata = (ft_rx.data[0]+ft_rx.data[1]*256+ft_rx.data[2]*256*256+ft_rx.data[3]*256*256*256);
